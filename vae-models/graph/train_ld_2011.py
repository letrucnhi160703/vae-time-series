...
def create_sliding_windows(data, window_length=12, predict_steps=1):
    X_data, Y_data = [], []
...
data, feature_names = load_csv_dataset(file_path)
...
# threshold = np.percentile(X_data, percentile, axis=0)
...
X_train = scaler_X.fit_transform(X_train.reshape(-1, num_features)).reshape(-1, window_length, num_features)
y_train = scaler_y.fit_transform(y_train.reshape(-1, num_features)).reshape(-1, predict_steps, num_features)

X_test = scaler_X.transform(X_test.reshape(-1, num_features)).reshape(-1, window_length, num_features)
y_test = scaler_y.transform(y_test.reshape(-1, num_features)).reshape(-1, predict_steps, num_features)

# Scale threshold
# threshold_array = np.tile(threshold, (1, window_length))  # Expand to (num_features, window_length)
# threshold = scaler_X.transform(threshold_array)[0]  # Normalize
# print(f"Threshold at Q {percentile}% for each feature: {threshold}")
...
vae = VAE(input_dim=input_dim, lstm_output_dim=output_dim, gcn_output_dim=0, latent_dim=latent_dim, future_steps=predict_steps, use_d_knn=False, use_gcn=False, use_gpd=False, use_bernoulli=False)
...
for epoch in range(epochs):
    ...
    for X_batch, y_batch in train_loader:
        ...
        loss = vae.loss_function(forecasting, y_batch, z_mean_normal, z_log_var_normal, None, z_scale_extreme, z_shape_extreme, z_logits_zero)
        loss.backward()
        optimizer.step()
