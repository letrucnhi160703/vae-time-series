...
def main(args):
    with open(args.config_filename) as f:
        ...
        # print("Number of missing nodes:", num_missing_nodes)
        # print("Number of available nodes:", num_available)
        ...
        for epoch in range(epoch_num):
            ...
            for _, (x, y) in enumerate(train_iterator):
                        ...
                        for b in range(batch_size):
                            for node_idx in range(num_nodes):
                                if X_mask[t, b, node_idx]:
                                    ...
                                    # if torch.sum(weights) == 0 or torch.isnan(x_imputed_val):
                                    #     print("?? Invalid imputation at t={}, b={}, node_idx={}, sum_w={}, weights={}".format(
                                    #         t, b, node_idx, torch.sum(weights).item(), weights))
                ...
                #     print("X_imputed has nan!!!!")
                # if not torch.isnan(X_imputed).any():
                #     print("X_imputed has no nan!!!!")

                forecasting, z_mean_normal, z_log_var_normal,z_scale_extreme, z_shape_extreme, z_logits_zero, xi_extreme_pred, sigma_extreme_pred = vae(x=X_imputed, y=y, batches_seen=batches_seen)
                # print("forecasting shape: ", forecasting.shape)
                if batches_seen == 0:
                    optimizer = torch.optim.Adam(vae.parameters(), lr=args.lr)

                loss, loss_gpd = vae.loss_function(forecasting, y, z_mean_normal, z_log_var_normal, _data['threshold'],
                                         z_scale_extreme, z_shape_extreme, z_logits_zero, x_full=x, x_imputed=X_imputed,
                                         mean_0=_data['x_train_mean'], log_var_0=_data['x_train_log_var'], 
                                         xi_extreme_pred=xi_extreme_pred, sigma_extreme_pred=sigma_extreme_pred)

                # print("_data['x_train_mean']: ", _data['x_train_mean'])
                # print("_data['x_train_log_var']: ", _data['x_train_log_var'])
                ...
                if loss_gpd:
                    ...
                    gpd_epoch_loss += loss_gpd.item() * x.size(0)
                total_samples += x.size(0)

                pot_loss = _compute_pot_loss(y, forecasting, standard_scaler, _data['threshold'])
                pot_losses.append(pot_loss.item())

                # z_shape_extreme_clone = z_shape_extreme.detach().numpy()

                # z_shape.append(z_shape_extreme_clone)
            ...
            # z_shape_mean = np.mean(z_shape)
            z_shape_mean = 0
            # print(f"Epoch {epoch + 1}/{epoch_num}, Loss: {epoch_loss / total_samples:.4f}, POT loss: {average_pot_loss}, GPD Loss: {gpd_epoch_loss / total_samples:.4f}, z_shape_extreme mean: {z_shape_mean}")
            print(f"Epoch {epoch + 1}/{epoch_num}, Loss: {epoch_loss / total_samples:.4f}, POT loss: {average_pot_loss}, GPD Loss: {gpd_epoch_loss / total_samples:.4f}")
        
        # Save model
        torch.save(vae, "dknn_miss10_thres99.pt")
        print("Full model saved to vae_model.pt")
        ...
        for x, y in test_iterator:
            ...
            predictions, _, _, _, _, _, _, _ = vae(x=X_imputed, y=None, batches_seen=batches_seen)

            # print("truth mean: ", y[..., 0].mean())
            # print("truth std: ", y[..., 0].std())
            # print("pred mean: ", predictions[..., 0].mean())
            # print("pred std: ", predictions[..., 0].std())
            ...
            y_true = standard_scaler.inverse_transform(y)
            y_predicted = standard_scaler.inverse_transform(predictions)
        ...
        y_preds = standard_scaler.inverse_transform(y_preds)
        ...
        # print("Mean Truth:", y_truth_series.mean().item())
        # print("Mean Pred :", y_pred_series.mean().item())
